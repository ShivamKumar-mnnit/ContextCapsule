...
// 60 requests/min per API key — standard reads/writes
...
// 10 LLM operations/min per API key — ingest, query, lint are expensive
export const rateLimitLlmOps = createMiddleware(async (c, next) => {
  const apiKeyRecord = (c as any).get('apiKeyRecord') as { id: string } | undefined
  if (!apiKeyRecord) {
    await next()
    return
  ...
  const result = checkRateLimit(`llm:${apiKeyRecord.id}`, 10, 60_000)
  setRateLimitHeaders(c, result)

  if (!result.allowed) {
    return errorResponse(c, 429, 'rate_limited', 'LLM operation limit reached. Try again in a minute.')
  }
